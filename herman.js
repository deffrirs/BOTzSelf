...
const dtod = "6282132805286@s.whatsapp.net"

const otod = "6282132805286@s.whatsapp.net"
const ktod = "6282132805286@s.whatsapp.net"

const htod = "6282132805286@s.whatsapp.net"
...
//━━━━━━━━━━━━━━━[ CONNECTION 1 ]━━━━━━━━━━━━━━━━━//
		...
		faketeks = '©Deffri メ Gans ×͜×' //Ganti Aja Nama Luh
		const isUrl = (url) => {
...
//━━━━━━━━━━━━━━━[ BUTTON ]━━━━━━━━━━━━━━━━━//
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = simple.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "List DEFFBOTz","footerText": `_HELLO KAK_ >.<
✋ 
...
"body": `hy`, 
"mediaType": "2", 
"mediaUrl": "https://youtube.com/shorts/-lXMUYBfB7Q?feature=share", 
...
simple.updatePresence(from, Presence.recording)
   ...
   sendButImage(from, pll, `Deffri メ Gans ×͜× © 2021`, cewe, [                      
          {
...
//>>>>>>>>>>>>>>>>>>>>[ COMMAND ]<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<//
              ...
              simple.updatePresence(from, Presence.recording)
               list = []
            listmenu = [`shop`,`sewabot`,`groupmenu`,`stickermenu`,`apkmod`,`downloadmenu`,`islammenu`,`animemenu`,`informationmenu`,`randomtextmenu`,`searchmenu`,`primbonmenu`,`stalkmenu`,`randomimagemenu`,`ownermenu`,`asupanmenu`,`gabut`,`game`,`meme`,`murid`,`djmenu`,`makermenu`,`walisongo`,]
               listmenuu = [`shop`,`sewabot`,`grupmenu`,`stickermenu`,`apkmod`,`downloadmenu`,`islammenu`,`animemenu`,`informationmenu`,`randomtextmenu`,`searchmenu`,`primbonmenu`,`stalkmenu`,`randomimagemenu`,`ownermenu`,`asupanmenu`,`gabut`,`game`,`meme`,`murid`,`djmenu`,`makermenu`,`walisongo`,]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: '${ownername}',
                    rows: [
                       {
                        title: `${listmenuu[startnum++]}`,
                        description: `SUBS DEFFBOTz`,
                        rowId: `${prefix}${x}`
                      }
                    ]
                   }
...
[`sholawat1`,`sholawat2`,`sholawat3`,`sholawat4`,`sholawat5`,`sholawat6`,`sholawat7`,`sholawat8`,`sholawat9`,] 
               ...
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: 'Deffri メ Gans ×͜× ' + nombor++,
                    rows: [
                       {
                        title: `${listmenuu[startnum++]}`,
                        description: ``,
...
trans = `[ 𝗜𝗡𝗙𝗢 𝗕𝗢𝗧 ]
🤖𝐍𝐚𝐦𝐚 𝐁𝐨𝐭 : ${botname}
😎𝐂𝐫𝐞𝐚𝐭𝐨𝐫 𝐁𝐨𝐭 : ${ownername}
👻𝐍𝐚𝐦𝐚 𝐎𝐰𝐧𝐞𝐫 : ${ownername}
📱𝐍𝐨𝐦𝐞𝐫 𝐎𝐰𝐧𝐞𝐫 : ${owner}
...
costum(buffer, sticker, Verived, `Deffri メ Gans ×͜×`)
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
buffer = fs.readFileSync(ran)
costum(buffer, image, Verived, `©_Deffri メ Gans ×͜×_ *V : 2.889H*`)
fs.unlinkSync(ran)
})
break
case 'tomp3':
...
fs.unlinkSync(anum)
buffer = fs.readFileSync(ran)
simple.sendMessage(from, buffer, video, { quoted: ftrol, caption: 'Done... Deffri メ Gans ×͜×' })
fs.unlinkSync(ran)
})
break
...
simple.sendMessage(`6282132805286@s.whatsapp.net`, options, text, { quoted: ftrol })
...
simple.sendMessage(from, buffer, image, {quoted: ftrol, caption: 'Deffri メ Gans ×͜×'})
break
case 'hijaber':
...
reply(mess.wait)
buffer = await getBuffer(ini.result.url)
simple.sendMessage(from, buffer, video, {quoted: ftrol, caption: 'Deffri メ Gans ×͜×'})
break
case 'ukhti':
...
var ttrth = (await fetchJson(`https://zeroyt7-api.xyz/truth?apikey=R9I4zuPC`)).message
var bff = Buffer.alloc(0)
simple.sendMessage(from, `_*Truth*_\n\n${ttrth}`, `${tampilUcapan}`, fs.readFileSync("./poto/herman.jpg"), { contextInfo: { mentionedJid: [sender] }})
break
case 'dare':
simple.updatePresence(from, Presence.recording)
...
var der = (await fetchJson(`https://zeroyt7-api.xyz/truth?apikey=R9I4zuPC`)).message
var bff = Buffer.alloc(0)
simple.sendMessage(from, `_*Truth*_\n\n${ttrth}`, `${tampilUcapan}`, fs.readFileSync("./poto/herman.jpg"), { contextInfo: { mentionedJid: [sender] }})
break
case 'tebakkalimat':
...
but = [
        ...
        sendButton(from, teks, '©Created : Deffri メ Gans ×͜×', but, mek)
break
 ...
 if (args.length == 0) return reply(`Contoh: ${prefix + command} awok awok`)
                    ...
                    getBuffer(`https://api.lolhuman.xyz/api/textprome2/${command}?apikey=Z-BOT8&text1=${txt1}&text2=${txt2}`).then((gambar) => {
                        simple.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `©Deffri メ Gans ×͜× _IMAGE_`, quoted : ftrol})
                    })
                    break
           ...
           case 'cup':
            ...
            simple.updatePresence(from, Presence.recording)
                    ...
                    getBuffer(`https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${humankey}&text=${ini_txt}`).then((gambar) => {
                        simple.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `©Deffri メ Gans ×͜×`, quoted : ftrol})
                    })
                    break
                case 'arcade8bit':
                case 'battlefield4':
...
case 'pdf':
        ptod = "6282132805286@s.whatsapp.net"
        stod = `${sender}`
        uwu = '```'
       stst = await simple.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
tescok = `${menunya(prefix)}`
Sendbutdocument(from, tescok, "© Deffri メ Gans ×͜×*", fs.readFileSync('./poto/menu.pdf'), {mimetype:Mimetype.pdf, thumbnail:fs.readFileSync('./poto/image/thumbnaildokumen.jpg'), filename:`DEFFBOTz.pdf`, pageCount: 9999999 },[{ buttonId: `${prefix}owner`, buttonText: { displayText: 'OWNER🌺' }, type: 1 }], {quoted:fvid, contextInfo: { mentionedJid: [dtod,otod,stod], forwardingScore: 508, isForwarded: true, externalAdReply:{title:`${tampilUcapan} - ${pushname}\nDownload Script`,mediaType:"2",thumbnail:ofrply,mediaUrl:`https://youtu.be/x-O0WHkv3uc`}}})
break
